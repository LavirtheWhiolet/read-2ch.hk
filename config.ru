...
class Read2ch_hk
  ...
  def forward_to_2ch_hk_and_unhide_some_content(env)
    unhiding_cookie = "usercode_auth=9a35febd6ebdce6c5da33017f15c7960"
    env =
      {
...
<div class="reply">
  ...
  <div class="post_file"><a href="<%=post_file.path%>" target="_blank"><img src="<%=post_file.thumbnail%>" name="<%=post_file.name%>"/></a></div>
  <% end %>
